...
def register():
    ...
    try:
        c.execute(
            "INSERT INTO users (id, username, password, ssn, dob, address) VALUES (?, ?, ?, ?, ?, ?)",
...
def get_credit_score(ssn):
    ...
    score = int(hashlib.sha256(ssn.encode()).hexdigest(),
                16) % 601 + 300  # Generates a score between 300 and 900
    return score


def has_existing_loans(user_id):
    conn = sqlite3.connect('loans.db')
    c = conn.cursor()
    c.execute(
        """SELECT COUNT(*) FROM loan_applications 
                 WHERE user_id = ? AND status = 'Approved' AND balance > 0""",
        (user_id, ))
    count = c.fetchone()[0]
    conn.close()
    return count
...
def apply_loan(current_user):
    ...
    conn.row_factory = sqlite3.Row
    c = conn.cursor()
    c.execute("SELECT ssn, dob, address FROM users WHERE id = ?",
              (current_user, ))
    ...
    if not user_info['ssn']:
        ...
        c.execute(
            "UPDATE users SET ssn = ?, dob = ?, address = ? WHERE id = ?",
            (ssn, dob, address, current_user))
        conn.commit()
    else:
    ...
    # Determine loan status based on a complex approval formula
    existing_loans = has_existing_loans(current_user)
    max_loan_amount = income * 0.4 - existing_loans * 50000  # Example formula

    if credit_score >= 700 and amount <= max_loan_amount and existing_loans < 3:
        status = 'Approved'
    else:
        status = 'Rejected'
    ...
    c.execute(
        """INSERT INTO loan_applications 
                 ...
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (loan_id, current_user, name, amount, purpose, credit_score, income,
         status, balance))
    conn.commit()
    conn.close()

    return jsonify({
        "message": f"Loan application {status.lower()}",
        "loan_id": loan_id
    }), 201
...
def get_applications(current_user):
    ...
    c = conn.cursor()
    c.execute(
        """SELECT id, name, amount, purpose, credit_score, income, status, balance 
...
def get_loans(current_user):
    ...
    c = conn.cursor()
    c.execute(
        """SELECT id, amount, balance FROM loan_applications 
                 WHERE user_id = ? AND status = 'Approved' AND balance > 0""",
        (current_user, ))
    loans = []
    for row in c.fetchall():
...
def make_payment(current_user):
    ...
    c.execute(
        """SELECT balance FROM loan_applications 
                 WHERE id = ? AND user_id = ? AND status = 'Approved'""",
        (loan_id, current_user))
    result = c.fetchone()
    if not result:
        conn.close()
        return jsonify(
            {"message": "Loan not found or not eligible for payment"}), 400
    ...
    if payment_amount > current_balance:
        conn.close()
        return jsonify(
            {"message": "Payment amount exceeds outstanding balance"}), 400

    # Update the loan balance
    new_balance = current_balance - payment_amount
    c.execute("UPDATE loan_applications SET balance = ? WHERE id = ?",
              (new_balance, loan_id))
    ...
    c.execute(
        "INSERT INTO loan_payments (id, loan_id, amount, date) VALUES (?, ?, ?, ?)",
...
def payment_history(current_user, loan_id):
    ...
    c.execute("SELECT id FROM loan_applications WHERE id = ? AND user_id = ?",
              (loan_id, current_user))
    if not c.fetchone():
        conn.close()
    ...
    c.execute(
        "SELECT amount, date FROM loan_payments WHERE loan_id = ? ORDER BY date DESC",
        (loan_id, ))
    payments = []
    for row in c.fetchall():
        payment = {"amount": row["amount"], "date": row["date"]}
